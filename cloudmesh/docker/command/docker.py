...
<<<<<<< HEAD
    ...
    * This function is idempotent, meaning it's ignored
    ...
    def deploy_docker(self, hosts, force):
        ...
        if not working_hosts:
            print('Failed to connect to all of the '
                  'provided hosts. Deploy aborted.')
        else:
            target_hosts = working_hosts if force else self.get_target_hosts(
                working_hosts)
            if not target_hosts:
                print('Docker is already installed on all of '
                      'the provided hosts. Deploy aborted.')
            else:
    ...
    * Assumes get-docker.sh already exists in each target 
    ...
    def cleanup_docker(self, hosts):
        ...
        Host.ssh(hosts, command)
        print('Success! Installed Docker on hosts '
